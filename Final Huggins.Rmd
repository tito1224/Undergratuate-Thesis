...
dfEstimatesFinal = left_join(dfEstimatesFinal,params,by=c("combinationNumber"="Scenario"))
...
```{r}
dfSummarisedFinal = dfEstimatesFinal %>%
...
ggplot()+
  ...
  facet_wrap(~maxMinute,scales="free")+ 
  scale_fill_discrete(name = "Alpha")+ 
  labs(x= "Per Minute Detection Probability",y = "Estimate")+
  theme_bw()
```
...
dfEstimatesFinal%>%
  ...
  theme_bw()+
  labs(title = "Interaction Plot (Capture Occasion & Detection Probability)",subtitle = "Facetted by Alpha",x = "Number of Capture Occasions",y = "Mean Population Estimate")+
...
```{r}
dfSummaryStats %>%
  filter(N==20 & p_1m %in% c(0.4)&alpha %in% c(0,0.05) & maxMinute == 10)%>%
  select(p_1m, alpha, N, maxMinute, AvgNhat, AvgNhatSE, biasNhat, mse, coverage)
```
