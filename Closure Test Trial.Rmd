...
```{r}
# function to detect time of last occurence
...
# test cases
dfHistoryFinalH[testGreaterOne[1:5],c(colnamesUse,"bCounts","counts")]
apply(dfHistoryFinalH[testGreaterOne[1:5],c(colnamesUse,"bCounts","counts")]==1,1,function(x) match(TRUE,x))
apply(dfHistoryFinalH[testGreaterOne[1:5],c(colnamesUse,"bCounts","counts")],1,detectMin)
...
```{r}
otisDetectClosure = function(df = dfHistoryFinalH){
  
  vi =  apply(df[,c(colnamesUse,"bCounts","counts")]==1,1,function(x) match(TRUE,x)) # occasion of first capture 
  
  # wi is the occasion of last capture - set to 0 for now **ASK**
  wi = apply(df[,c(colnamesUse,"bCounts","counts")],1,detectMin)
  
  qi = vi = wi # waiting time between first and last capture 
}
```
