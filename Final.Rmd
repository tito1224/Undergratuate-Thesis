...
```{r}
ll_func = function(N,p=0.1){
  #val = log(factorial(10)/factorial(7)) + 3*log(p) + 47*log(1-p)
  val = (factorial(N)/factorial(N-3))*(p**3)*((1-p)**((N*5)-3))
  return(log(val))
}

ll_data = tibble(N = seq(0,210,0.1),
                 log_likelihood = ll_func(N,p=0.5))

ggplot(ll_data)+
  geom_line(aes(x= N, y = log_likelihood))+ 
  theme_bw()+ 
  labs(title = "Log-Likelihood for 3 Individuals Detected Only Once", subtitle = "p = 0.1, Capture Occasions = 5", x= "N", y = "Likelihood")
```
...
dfPointCounts%>%
  filter(N == 20)%>%
ggplot()+
  geom_boxplot(aes(x=as.factor(p_1m), y = PointCounts, color = as.factor(alpha)))+
  facet_wrap(~maxMinute)+
  theme_bw()+
  labs(title = "Point Counts Across Scenarios Facetted by Capture Ocassions", subtitle = "Population N = 20", x= "P_1m", y = "Point Counts")+ 
...
```{r}
ggplot(dfCounts,aes(x=as.factor(counts), y = percent, fill = as.factor(alpha)))+ 
...
ggplot(aes(x=p_1m, y = AvgNhat, color = as.factor(alpha)))+ 
  ...
  facet_wrap(~as.factor(alpha))+
  theme_bw()+ 
  labs(title  = "Average Population Estimates and Standard Error", subtitle = "N = 20, 5 Capture Occasions.",x= "1 Min Detection Probability (p_1m)",y = "Average Population Estimate")+
  scale_color_discrete(name = "Alpha")
```
...
ggplot(aes(x=p_1m, y = mse, color = as.factor(alpha)))+ 
  ...
  facet_wrap(~maxMinute)+
  theme_bw()+ 
  labs(title  = "Mean Squared Error of Population Estimates", subtitle = "Facetted by Capture Occasions. N = 20. ",x= "1 Min Detection Probability (p_1m)",y = "Average MSE of Estimates")+
  scale_color_discrete(name = "Alpha")
```
```{r}
dfSummaryStats %>%
...
ggplot(aes(x=p_1m, y = coverage, color = as.factor(alpha)))+ 
  ...
  facet_wrap(~maxMinute)+
  theme_light()+ 
  labs(title  = "Coverage Probability", subtitle = "Facetted by Capture Occasions. N = 20.", x= "1 Min Detection Probability (p_1m)",y = "Coverage Probability")+
  scale_color_discrete(name = "Alpha")+
  theme_bw()
```
...
dfSummaryStats %>%
  filter(N==20)%>%
ggplot(aes(x=p_1m, y = avgWidth , color = as.factor(alpha)))+ 
  geom_point()+
  geom_line()+
  facet_wrap(~maxMinute)+
  theme_bw()
