...
```{r}
generateLatentMatrix_sjb <- function(dfRegular,alpha,maxMinute,seed = NULL){
  ...
  # Simulate movements
  movements <- rbinom(nrow(dfRegular) * (maxMinute - 1), 1, alpha) %>%
    matrix(nrow = nrow(dfRegular)) 
  colnames(movements) <- 1:(maxMinute - 1) 
  
  # Add individual and location identifiers
  movements <- dfRegular %>%
    select(individuals, locationID, UniqueIdentifier) %>%
    bind_cols(movements)
  
}

# for reference so we can see what latent matrix is used to create errors
latentMatrixUsed = generateLatentMatrix_sjb(dfRegular,alpha,maxMinute,1)
latentMatrixUsed
```
...
similar to the distribution of N_i, but the density at certain points is lower. The 
...
I want to see how the theoretical probability of detection compares against the detection
probability implied by the data. Obviously as probability of an error increases, 
we will see an implied detection probability > 1.

```{r}
dfError_cumulative_location = left_join(dfError_cumulative,dfError_Summarized[,c("N_i","locationID")],
                               by = "locationID")
colsUse = as.character(1:maxMinute)
CountByTimeInterval_e = dfError_cumulative_location %>%
...
```{r}
# find theoretical probability of being detected at least once for erroneous data
summaryPhat = CountByTimeInterval_e %>%
  group_by(TimeInterval)%>%
  summarise(p_data_error = mean(p_hat))
...
```{r}
# find theoretical probability of being detected at least once for regular data
dfRegular_Cumulative_location = left_join(dfRegular_Cumulative,dfRegular_Summarized[,c("N_i","locationID")],
                               by = "locationID")
colsUse = as.character(1:maxMinute)
CountByTimeInterval_r = dfRegular_Cumulative_location %>%
  pivot_longer(cols = c(colsUse), names_to = "TimeInterval", values_to = "Count")%>%
  group_by(locationID,TimeInterval) %>%
  mutate(TimeInterval = as.numeric(TimeInterval)) %>%
  summarise(Count = sum(Count > 0), # find number of individuals detected at least once
...
CountByTimeInterval_r
summaryRegularPHat = CountByTimeInterval_r %>%
 group_by(TimeInterval)%>%
  summarise(p_data_reg = mean(p_hat))
summaryRegularPHat

summaryPhat$p_data_reg = summaryRegularPHat$p_data_reg
```


```{r}
summaryPhat %>%
  pivot_longer(cols=c("p_data_error","p_data_reg","p_theoretical"),names_to = "Type",values_to = "p" )%>%
  filter(Type!="p_data_error")%>%
ggplot()+
