...
Find the implied p_hat of the erroneous data.
```{r}
dfError_cumulative = left_join(dfError_cumulative,dfError_Summarized[,c("N_i","locationID")],
                               by = "locationID")
colsUse = as.character(1:maxMinute)
CountByTimeInterval = dfError_cumulative %>%
...
dfRegular_Cumulative = left_join(dfRegular_Cumulative,dfRegular_Summarized[,c("N_i","locationID")],
                               by = "locationID")
colsUse = as.character(1:maxMinute)
CountByTimeInterval = dfRegular_Cumulative %>%
  pivot_longer(cols = c(colsUse), names_to = "TimeInterval", values_to = "Count")%>%
  group_by(locationID,TimeInterval) %>%
  mutate(TimeInterval = as.numeric(TimeInterval)) %>%
  summarise(Count = sum(Count > 0), # find number of individuals detected at least once
...
summaryRegularPHat = CountByTimeInterval %>%
  group_by(TimeInterval)%>%
