...
```{r}
fitModel(dfMark, bC = TRUE, bMixture =TRUE, bAdditive = TRUE)
```

vs interaction:
```{r}
fitModel(dfMark, bC = TRUE,bMixture=TRUE, bAdditive = FALSE)
```
...
```{r}
dfSimulation1 = calculateStatistics(nRuns = 10, lstNi = c(100), lstP = c(0.2), lstAlpha = c(0,0.05,0.1,0.2,0.3),lstMaxMin = c(5),seed=10)
dfSimulationEst = dfSimulation1[[1]]
dfSimulationParams= dfSimulation1[[2]]%>%
  filter(variable == "N")
dfSimulationParams_P = dfSimulation1[[2]]%>%
