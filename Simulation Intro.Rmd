...
`fitModel()` basically just uses mark() function to find estimates-> returns p and N estimates
```{r}
# true population
sum(dfPop$N_i)
```
...
```{r}
fitModel(dfMark,strFormula = "~1",strModel="Closed")
```
...
- runSingleSimulation will generate a dataframe of detection histories using the given population value (N_i) and uses fitModel() to reutrn the p and N estimates, along with the se, confidence interval,aic
- it also returns the parameters used as input in the function call, and the detection history that was generated (in long format)
```{r}
