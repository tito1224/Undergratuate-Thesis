...
ggplot()+ 
  ...
  scale_color_discrete(name="Alpha")+
  theme_bw()+
  labs(title = "Results of Otis Closure Test", subtitle = "Significance Level of 0.05", x = "Per minute detection probability", y ="p Otis")
```
...
nTotal = nrow(filter(dfClosure_CJS, N == 20 & maxMinute == 10 & !duplicated(id)))
dfClosure_CJS%>%
  filter(N == 20 & maxMinute == 10 & !duplicated(id))%>%
  summarise(nOne = sum(pExactOne)/nTotal)

dfClosure_Grouped = dfClosure_CJS%>%
   filter(N == 20 & maxMinute == 10 & !duplicated(id))%>%
  group_by(p_1m,alpha)%>%
  summarise(pExactOne = n())

dfClosure_CJS%>%
  filter(N == 20 & maxMinute == 10 & !duplicated(id))%>%
  group_by(p_1m,alpha)%>%
  summarise(percentPvalOne = sum(pExactOne)/1000)
```
